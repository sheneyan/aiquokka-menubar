...
DeepSeek provider 摘要在没有百分比时显示余额，而不是“没有可用的使用率窗口”。若同一个 provider 有多个余额窗口，摘要优先以 USD、再以 CNY 的固定顺序用斜线并列展示，例如 `$0.00 / ￥95.02`；只有一种货币时仅显示该金额。该摘要在折叠状态即可见，不要求展开详情或手动刷新。菜单栏全局摘要仍显示其他 provider 的最高真实百分比；只有余额数据时不在菜单栏伪造百分比。
